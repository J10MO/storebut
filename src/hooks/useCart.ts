    ...
    (set, get) => ({
        ...
        try {
          ...
          if (product.stock_quantity !== undefined && product.stock_quantity !== null) {
            if (newQuantity > product.stock_quantity) {
              throw new Error(`الكمية المتوفرة فقط ${product.stock_quantity} قطعة`)
            }
          }
          ...
          if (existingItem) {
            ...
            toast.success(`تم تحديث الكمية 🛒`, {
              description: `${product.name} - الكمية الجديدة: ${newQuantity}`,
              duration: 3000,
            })
          } else {
            updatedItems = [...currentItems, { product, quantity }]
          ...
          if (token) {
            try {
              console.log("Syncing with server...", { productId: product.id, quantity: newQuantity })
              await cartAPI.addToCart(product.id, quantity)
              console.log("Product successfully synced with server")
            } catch (serverError: any) {
              console.error("خطأ في مزامنة السلة مع الخادم:", serverError)
      ...
      updateQuantity: async (productId: number, quantity: number) => {
        ...
        try {
          ...
          if (item.product.stock_quantity !== undefined && item.product.stock_quantity !== null) {
            if (quantity > item.product.stock_quantity) {
              toast.error("الكمية غير متوفرة", {
                description: `الكمية المتوفرة فقط ${item.product.stock_quantity} قطعة`,
                duration: 4000,
              })
              return
            }
