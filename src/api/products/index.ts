...
export const productsAPI = {
  ...
  searchProducts: async (query: string, params?: Omit<ProductSearchParams, "q">): Promise<ProductsResponse> => {
    const response = await apiClient.get<ProductsResponse>("/products/search", {
      params: { 
        q: query,
        page: params?.page || 1,
  ...
  getByCategory: async (categoryId: string | number, params?: { page?: number; limit?: number }): Promise<ProductsResponse> => {
    const response = await apiClient.get<ProductsResponse>(`/categories/${categoryId}/products`, {
      params: {
        page: params?.page || 1,
        limit: params?.limit || 10,
  ...
  // Rate a product
  rateProduct: async (productId: string | number, rating: number, comment?: string): Promise<any> => {
    const response = await apiClient.post(`/products/${productId}/rate`, {
      rating,
      comment,
    })
  ...
  // Get product ratings
  getProductRatings: async (productId: string | number, page = 1, limit = 10): Promise<any> => {
    const response = await apiClient.get(`/products/${productId}/ratings`, {
      params: { page, limit },
    })
    return response.data
