...
// //             email: email.trim() || `${cleanPhone}@example.com`,
// //             address: {
// //               street: address.landmark, // اقرب نقطة دالة
// //               city: address.governorate, // المحافظة (بغداد)
// //               district: address.district, // المنطقة او القضاء
// //               postalCode: address.area, // الحي
// //             },
// //           }
...
// //                     onChange={(e) => setCode(e.target.value.replace(/\D/g, ""))}
// //                     placeholder="أدخل رمز التحقق المكون من 4 أرقام"
// //                     className="pr-12 text-center tracking-widest text-xl font-mono h-14 border-2 border-gray-200 focus:border-amber-500 rounded-xl dark:bg-gray-700 dark:border-gray-700 dark:focus:border-amber-400"
// //                     disabled={loading}
// //                     maxLength={4}
// //                     dir="ltr"
// //                   />
...
//             email: email.trim() || `${cleanPhone}@example.com`,
//             address: {
//               street: address.landmark, // ��قرب نقطة دالة
//               city: address.governorate, // المحافظة (بغداد)
//               district: address.district, // المنطقة او القضاء
//               postalCode: address.area, // الحي
//             },
...
const Login: React.FC<LoginProps> = ({ showCloseButton = true, onClose }) => {
  ...
  const handleSendOTP = async () => {
    ...
    const cleanPhone = phone.replace(/\s/g, "")
    const phoneRegex = /^(077|075|078|079)[0-9]{8}$/

    if (!phoneRegex.test(cleanPhone)) {
      setError("رقم الهاتف يجب أن يبدأ بـ 077 أو 075 أو 078 أو 079 ويحتوي على 11 رقماً")
      return
    }
    ...
    if (
      ...
      setError("رقم الهاتف يجب أن يبدأ بـ 077 أو 075 أو 078 أو 079")
      return
    }
  ...
  const formatPhoneNumber = (value: string) => {
    ...
    if (
      (numbers.startsWith("077") ||
        numbers.startsWith("075") ||
        numbers.startsWith("078") ||
        numbers.startsWith("079")) &&
      numbers.length > 3
    ) {
      formatted = numbers.slice(0, 11)
    } else if (numbers.startsWith("07") && numbers.length > 2) {
      formatted = `07${numbers.slice(2, 11)}`
    }

    // Format: 07X XXX XXXX
    if (formatted.length > 6) {
      formatted = formatted.replace(/(\d{3})(\d{3})(\d{0,5})/, "$1 $2 $3")
            ...
            <div className="space-y-6">
              <div className="bg-blue-50/50 border border-blue-200 rounded-2xl p-5 dark:bg-gray-800 dark:border-gray-700">
                ...
                <div className="flex justify-between items-center mt-3">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">مثال: 077 999 9999</p>
                </div>
              </div>
